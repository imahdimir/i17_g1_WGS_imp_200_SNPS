...
df_plus <- df %>% filter(model_name == "plus", term == "g1_minus_g2_imp_dsg")
...
combined_plot <- ggplot(df_plus, aes(x = estimate, fill = quality)) +
  ...
  scale_color_manual(name = "Vertical Lines", values = vline_colors, guide = guide_legend(override.aes = list(
    ...
    size = .6  # Custom height of short lines in the legend
  ))) +
  labs(title = "Combined Histogram for Plus Models (g1_minus_g2_imp_dsg)", x = "Estimate", y = "Frequency") +
  theme_minimal() +
  theme(
